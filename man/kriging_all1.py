...
#Elements=["Al","Ar","As","Ca","Cl","Cu","Fe","Hg","K","Mn","Ni","P","Pb","Rh","S","Si","Sr","Ti","Zn"]

Elements=["Cu","Fe","Hg","Pb","S","Ti"]

Vec_num_points=[66,131,263,394,526,657,788,920,1051,1183]
...
def to_list(Mat_intensities):
	Intensities_result=[0.0]*Num_points
	Pos=0
	for i in range(len(Mat_intensities)):
		for j in range(len(Mat_intensities)):
...
def main():
	...
	# using the oroginal positions coordinates
	# Intensities_result=mhd(Coordinates_x,Coordinates_y,Colors,Intensities,Selected_pos)
	# Mat_intensities=to_mat(Intensities_result)

	# usinf the small position coordinates
	#Coordinates_x, Coordinates_y = matrix_positions()

	# Intensities_result1 = mhd(Coordinates_x, Coordinates_y, Colors, Intensities, Selected_pos)
	# Mat_intensities1 = to_mat(Intensities_result1)
		...
		for Model in Krigin_variogram_model:
			...
			for Num_random_pos in Vec_num_points:
				...
				for Num_test in range(Num_tests):
					...
					# grid or points
					Intensities_result, ss = Ordinary_kriging.execute("points", x_unknown, y_unknown)
					...
					for i in range(Num_points):
						if Valid[i]==False:
							MSE=MSE+math.pow(Intensities[i]-Intensities_result[Pos],2)
							Pos=Pos+1
			...
			save_time(Folder_time, Element, Vec_time)
					...
					# 	Selected_x[i] = Selected_pos[i] % Num_cols
					# 	Selected_y[i] = int(Selected_pos[i] / Num_cols)


					#draw_result(Element,Num_random_pos,Mat1,Selected_x,Selected_y)

					# draw_result("KRIGIN "+Model,Element, Num_random_pos, Mat_intensities1, Selected_x, Selected_y)
					#
					# draw_error("KRIGIN "+Model,Element,Num_random_pos,Mat_error_intensities,Selected_x,Selected_y)
