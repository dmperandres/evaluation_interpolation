...
Test_method="CRESSMAN"
Test_folder="cressman"
			...
			for Num_test in range(Num_tests):
				...
				for i in range(Num_points):
					if Valid[i]==False:
						...
						# get the distance for the selected point to the other that are in the circle
						cress_dist = dist_2(x_unknown[Pos], y_unknown[Pos], x2, y2)

						# get the values of the positions that are in the circle
						cress_obs = [Intensities[x] for x in Indices_true_points_in_circle[Pos]]

						# computes the cressman value
						cress_val = cressman_point(cress_dist, cress_obs, radius)

						Intensities_result[i] = cress_val
